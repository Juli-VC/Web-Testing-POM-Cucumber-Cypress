...
When("I check the {string} product on the list has the {string} equal to {string}", (positionList, attribute, attrValue) => {
    mainPage.checkPositionFirstOrLastHasAttributeEqualTo(positionList, attribute, attrValue);
});
...
When("I check that the product {int} in the position list has the {string} attribute with the {string} value", (productPositionList, productAttribute, attributevalue) => { 
  mainPage.checkValueOfProductByPositionOnTheList(productPositionList, productAttribute, attributevalue)
});
